...
for story_column, experiment_name in [('inanimate_story', experiment_name_base), ('animate_story', experiment_name_base + '_animate')]:
    ...
    for i, (s, w1, w2) in enumerate(zip(df[story_column], df['inanimate_adjective'], df['animate_adjective'])):
        s = s.strip()
    ...
    Path('results/boudewyn_exp2').mkdir(exist_ok=True, parents=True)
    output_df.to_csv(f'results/boudewyn_exp2/{experiment_name}.csv')
    ...
    with jsonlines.open(f'results/boudewyn_exp2/{experiment_name}.jsonl', mode='w') as writer:
